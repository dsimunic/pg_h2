...
-spec ping(pid(), ets:tid(), pg_http_pool:conn()) -> ok.
ping(Pid, Holder, _Conn) ->
    gen_statem:cast(Pid, {ping, Holder}).

-spec stop(pid(), ets:tid(), pg_http_pool:conn()) -> ok.
...
-spec reload_types(pg_http_pool:conn()) -> ok.
reload_types(#conn{owner=Pid}) ->
    gen_statem:call(Pid, reload_types).

-spec break(pg_http_pool:conn(), pg_http_pool:ref()) -> ok.
...
disconnected(EventType, _, Data=#data{broker=Broker,
    ...
    try pg_http_handler:open(Broker, PoolConfig) of
        {ok, Conn} ->
            Holder = pg_http_pool:update(Pool, QueueTid, ?MODULE, Conn),
...
handle_event(cast, {ping, Holder}, Data=#data{pool=Pool,
                                              ...
                                              conn=Conn}) ->
    %% TODO: do ping
    NewHolder = pg_http_pool:update(Pool, QueueTid, ?MODULE, Conn),
